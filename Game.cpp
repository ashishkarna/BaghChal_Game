...
bool Game::Start()
{
    ...
    App.Create(VideoMode(screenWidth, screenHeight, desktop.BitsPerPixel), "Baghchal", Style::Fullscreen);
    App.SetFramerateLimit(25);//limits the framerate. makes the program more efficient.

    //sets the icon in the titlebar
    App.SetIcon(sprites->img.goatimg.GetWidth(), sprites->img.goatimg.GetHeight(), sprites->img.goatimg.GetPixelsPtr());
